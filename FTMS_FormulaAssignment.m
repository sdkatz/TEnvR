...
if strcmpi(config.Ionization_Mode,'negative')
    ...
    Index_Refined=floor(DBE) == DBE & DBE>=config.DBE_min & DBE<=config.DBE_max; % Find combinations with a whole AND positive DBE value
...
elseif strcmpi(config.Ionization_Mode,'positive')
    ...
    index_refined1=NaK <= 1; % Find combinations with Na+K<=1
    ...
    index_refined2=floor(DBE) == DBE & DBE>=config.DBE_min & DBE<=config.DBE_max; %Find combinations with a whole AND positive DBE value
...
for i = 1:NumberOfPeaks
    ...
    if C_max <= max(FormulaCombinations_Refined(format.Row_C,:)) || C_max <= config.C_min
        ...
        if strcmpi(config.Ionization_Mode,'negative')
            ...
            Index_Refined_New=floor(DBE_New) == DBE_New & DBE_New>=config.DBE_min & DBE_New<=config.DBE_max; % Find combinations with a whole AND positive DBE value
        ...
        elseif strcmpi(config.Ionization_Mode,'positive')
            ...
            index_refined1_New=NaK_New <= 1; % Find combinations with Na+K<=1
            ...
            index_refined2_New=floor(DBE_New) == DBE_New & DBE_New>=config.DBE_min & DBE_New<=config.DBE_max; %Find combinations with a whole AND positive DBE value
